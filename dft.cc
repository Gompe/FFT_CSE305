...
void BaseDft::Transform(const Complex *x, Complex *out, int N, int stride){
    ...
    for(int k=0; k<N; k++){
        out[k] = 0;
        for (int n=0; n<N; n++){
            out[k] += x[stride*n] * fft_utils::RootOfUnity(N, -k*n);
        }
    }
}
...
void RecursiveFft::Transform(const Complex *x, Complex *out, int N, int stride) {
    if (!fft_utils::IsPowerOfTwo(N)) {
        throw std::logic_error("radix-2 can only be used if N is a power of 2");
    }

    RecursiveFft::TransformImpl(x, out, N, stride);
}

void RecursiveFft::TransformImpl(const Complex *x, Complex *out, int N, int stride) {
    // Implementation inspired by pseudocode in
    // https://en.wikipedia.org/wiki/Cooley%E2%80%93Tukey_FFT_algorithm

    if(N <= RecursiveFft::m_base_case_size) {
        BaseDft::Transform(x, out, N, stride);
        return;
    }
    ...
    RecursiveFft::TransformImpl(x, out, N/2, 2*stride);
    RecursiveFft::TransformImpl(x + stride, out + N/2, N/2, 2*stride);

    for (int k=0; k < N/2; k++) {
        Complex p = out[k];
        Complex q = fft_utils::RootOfUnity(N, -k) * out[k + N/2];
        out[k] = p + q;
        out[k + N/2] = p-q;
    }
}
...
{
    if (!fft_utils::IsPowerOfTwo(N)) {
        throw std::logic_error("The Iterative Implementation of FFT only supports "
                                "values of N that are powers of two.\n");
    }
    ...
    for (int s = 1; s <= logN; s++) {
        ...
        Complex twiddle = fft_utils::RootOfUnity(fft_utils::PowerOfTwo(s), -1);
